...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, struct_cond=None, seg_cond=None, lr_cond=None, org_lr=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer) or isinstance(layer, SpatialTransformerV2):
                assert context is not None
                x = layer(x, context, org_lr=org_lr)
            elif isinstance(layer, TimestepBlockDual):
                assert struct_cond is not None
                x = layer(x, emb, struct_cond, lr_cond=lr_cond)
...
class ResBlockDual(TimestepBlockDual):
    ...
    def forward(self, x, emb, s_cond, lr_cond=None):
        """
        Apply the block to a Tensor, conditioned on a timestep embedding.
        :param x: an [N x C x ...] Tensor of features.
        :param emb: an [N x emb_channels] Tensor of timestep embeddings.
        ...
        """
        return checkpoint(
            self._forward, (x, emb, s_cond, lr_cond), self.parameters(), self.use_checkpoint
        )


    def _forward(self, x, emb, s_cond, lr_cond=None):
        if self.updown:
            in_rest, in_conv = self.in_layers[:-1], self.in_layers[-1]
            h = in_rest(x)
            h = self.h_upd(h)
        ...
        if self.use_scale_shift_norm:
            ...
            h = h + emb_out
            h = self.out_layers(h)
        h = self.spade(h, s_cond, lr_cond=lr_cond)
        return self.skip_connection(x) + h
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, y=None, lr_cond=None, org_lr=None, **kwargs):
        """
        ...
        for module in self.input_blocks:
            h = module(h, emb, context, lr_cond=lr_cond, org_lr=org_lr)
            hs.append(h)
        h = self.middle_block(h, emb, context, lr_cond=lr_cond, org_lr=org_lr)
...
class UNetModelDualcondV2(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, struct_cond=None, y=None, lr_cond=None, org_lr=None, **kwargs):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        ...
        h = x.type(self.dtype)
        for module in self.input_blocks:
            h = module(h, emb, context, struct_cond, lr_cond=lr_cond, org_lr=org_lr)
            hs.append(h)
        h = self.middle_block(h, emb, context, struct_cond, lr_cond=lr_cond, org_lr=org_lr)
        for module in self.output_blocks:
            h = th.cat([h, hs.pop()], dim=1)
            h = module(h, emb, context, struct_cond, lr_cond=lr_cond, org_lr=org_lr)
        h = h.type(x.dtype)
...
class EncoderUNetModelWT(nn.Module):
    ...
    def forward(self, x, timesteps, name_list=None):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        ...
        if not name_list:
            ...
            for i in range(len(result_list)):
                results[name_list[i]] = self.fea_tran[i](result_list[i], emb)
