...
class LatentDiffusionSRTextWT(DDPM):
    ...
    def __init__(self,
        ...
        if not self.unfrozen_diff:
            ...
            for name, param in self.model.named_parameters():
                if 'spade' not in name and 'refer' not in name:
                    param.requires_grad = False
                else:
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        lr_h, lr_w = self.lq.size(-2), self.lq.size(-1)
        if lr_h < 128:
            lr_h = 128
        if lr_w < 128:
            lr_w = 128
        ...
        #     self.real_lq = F.interpolate(
        #         self.gt,
        #         size=(lr_h, lr_w),
        #         mode='bicubic'
        #     )
        # else:
        #     tmp_ratio = torch.rand(self.lq.shape[0], 1, 1, 1).to(self.device)
        #     self.real_lq = tmp_ratio * im_lq + (1-tmp_ratio) * F.interpolate(self.gt, size=(lr_h, lr_w), mode='bicubic')
        ...
        encoder_posterior_l = self.encode_first_stage(l)
        lr_con = self.get_first_stage_encoding(encoder_posterior_l).detach()
    ...
    def shared_step(self, batch, **kwargs):
        x, c, gt, xcon = self.get_input(batch, self.first_stage_key)
        loss = self(x, c, gt, xcon)
        return loss

    def forward(self, x, c, gt, xcon, *args, **kwargs):
        ...
        if self.test_gt:
            ...
            struc_l = self.structcond_stage_model(xcon, t_ori, list(struc_c.keys()))
        else:
            struc_c = self.structcond_stage_model(x, t_ori)
            struc_l = self.structcond_stage_model(xcon, t_ori, list(struc_c.keys()))
            
        return self.p_losses(gt, c, struc_c, t, t_ori, x, lr_con=struc_l, org_lr=xcon, *args, **kwargs)
    ...
    def apply_model(self, x_noisy, t, cond, struct_cond, return_ids=False, lr_con=None, org_lr=None):
    ...
    def p_losses(self, x_start, cond, struct_cond, t, t_ori, z_gt, noise=None, lr_con=None, org_lr=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        ...
        model_output = self.apply_model(x_noisy, t_ori, cond, struct_cond, lr_con=lr_con, org_lr=org_lr)
    ...
    def p_mean_variance(self, x, c, struct_cond, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
                        return_x0=False, score_corrector=None, corrector_kwargs=None, t_replace=None, struct_cond_lr=None, org_lr=None):
        if t_replace is None:
            t_in = t
        else:
            t_in = t_replace
        model_out = self.apply_model(x, t_in, c, struct_cond, return_ids=return_codebook_ids, lr_con=struct_cond_lr, org_lr=org_lr)
    ...
    def p_mean_variance_canvas(self, x, c, struct_cond, t, clip_denoised: bool, struct_cond_lr=None, return_codebook_ids=False, quantize_denoised=False,
                        return_x0=False, score_corrector=None, corrector_kwargs=None, t_replace=None, tile_size=64, tile_overlap=32, batch_size=4, tile_weights=None, scale=4):
        """
        ...
        _, _, h, w = x.size()
        _, _, h_lr, w_lr = struct_cond_lr.size()
        ...
        tile_size_lr = tile_size // scale
        tile_overlap_lr = tile_overlap // scale
        ...
        for row in range(grid_rows):
            ...
            for col in range(grid_cols):
                if col < grid_cols-1 or row < grid_rows-1:
                    ...
                    ofs_x_lr = max(row * tile_size_lr - tile_overlap_lr * row, 0)
                    ofs_y_lr = max(col * tile_size_lr - tile_overlap_lr * col, 0)

                if row == grid_rows-1:
                    ofs_x = w - tile_size
                    ofs_x_lr = w_lr - tile_size_lr
                if col == grid_cols-1:
                    ofs_y = h - tile_size
                    ofs_y_lr = h_lr - tile_size_lr
                ...
                input_start_x_lr = ofs_x_lr
                input_end_x_lr = ofs_x_lr + tile_size_lr
                input_start_y_lr = ofs_y_lr
                input_end_y_lr = ofs_y_lr + tile_size_lr
                ...
                cond_tile_lr = struct_cond_lr[:, :, input_start_y_lr:input_end_y_lr, input_start_x_lr:input_end_x_lr]
                cond_list_lr.append(cond_tile_lr)

                if len(input_list) == batch_size or col == grid_cols-1:
                    input_list = torch.cat(input_list, dim=0)
                    cond_list = torch.cat(cond_list, dim=0)
                    cond_list_lr = torch.cat(cond_list_lr, dim=0)

                    struct_cond_input = self.structcond_stage_model(cond_list, t_in[:input_list.size(0)])
                    struct_cond_input_lr = self.structcond_stage_model(cond_list_lr, t_in[:input_list.size(0)], list(struct_cond_input.keys()))
                    model_out = self.apply_model(input_list, t_in[:input_list.size(0)], c[:input_list.size(0)], struct_cond_input, lr_con=struct_cond_input_lr,
                                                 return_ids=return_codebook_ids, org_lr=cond_list_lr)
    ...
    def p_sample(self, x, c, struct_cond, t, clip_denoised=False, repeat_noise=False,
                 return_codebook_ids=False, quantize_denoised=False, return_x0=False,
                 temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None, t_replace=None, struct_cond_lr=None, org_lr=None):
        b, *_, device = *x.shape, x.device
        outputs = self.p_mean_variance(x=x, c=c, struct_cond=struct_cond, t=t, clip_denoised=clip_denoised,
                                       ...
                                       struct_cond_lr=struct_cond_lr, org_lr=org_lr)
        if return_codebook_ids:
            raise DeprecationWarning("Support dropped.")
            model_mean, _, model_log_variance, logits = outputs
        elif return_x0:
    ...
    @torch.no_grad()
    def p_sample_canvas(self, x, c, struct_cond, t, struct_cond_lr=None, clip_denoised=False, repeat_noise=False,
                 return_codebook_ids=False, quantize_denoised=False, return_x0=False,
        ...
        b, *_, device = *x.shape, x.device
        outputs = self.p_mean_variance_canvas(x=x, c=c, struct_cond=struct_cond, t=t, struct_cond_lr=struct_cond_lr, clip_denoised=clip_denoised,
                                       return_codebook_ids=return_codebook_ids,
                                       quantize_denoised=quantize_denoised,
                                       return_x0=return_x0,
                                       score_corrector=score_corrector, corrector_kwargs=corrector_kwargs, t_replace=t_replace,
    ...
    def p_sample_loop(self, cond, struct_cond, shape, return_intermediates=False,
                      x_T=None, verbose=True, callback=None, timesteps=None, quantize_denoised=False,
                      mask=None, x0=None, img_callback=None, start_T=None,
                      log_every_t=None, time_replace=None, adain_fea=None, interfea_path=None, lr_cond=None):
        ...
        for i in iterator:
            ...
            if t_replace is not None:
                ...
                struct_cond_lr = self.structcond_stage_model(lr_cond, t_replace, list(struct_cond_input.keys()))
            else:
                if start_T is not None:
                    if i > start_T:
                        continue
                struct_cond_input = self.structcond_stage_model(struct_cond, ts)
                struct_cond_lr = self.structcond_stage_model(lr_cond, ts, list(struct_cond_input.keys()))
            ...
            img = self.p_sample(img, cond, struct_cond_input, ts,
                                clip_denoised=self.clip_denoised,
                                quantize_denoised=quantize_denoised, t_replace=t_replace, struct_cond_lr=struct_cond_lr, org_lr=lr_cond)
    ...
    @torch.no_grad()
    def p_sample_loop_canvas(self, cond, struct_cond, shape, struct_cond_lr=None, return_intermediates=False,
                      x_T=None, verbose=True, callback=None, timesteps=None, quantize_denoised=False,
                      mask=None, x0=None, img_callback=None, start_T=None,
        ...
        for i in iterator:
            ...
            img = self.p_sample_canvas(img, cond, struct_cond, ts, struct_cond_lr=struct_cond_lr,
                                clip_denoised=self.clip_denoised,
    ...
    @torch.no_grad()
    def sample(self, cond, struct_cond, lr_cond=None, batch_size=16, return_intermediates=False, x_T=None,
               verbose=True, timesteps=None, quantize_denoised=False,
        ...
        return self.p_sample_loop(cond,
                                  struct_cond,
                                  shape, return_intermediates=return_intermediates, x_T=x_T,
                                  verbose=verbose, timesteps=timesteps, quantize_denoised=quantize_denoised, mask=mask,
                                  x0=x0, time_replace=time_replace, adain_fea=adain_fea, interfea_path=interfea_path, start_T=start_T, lr_cond=lr_cond)

    @torch.no_grad()
    def sample_canvas(self, cond, struct_cond, struct_cond_lr=None, batch_size=16, return_intermediates=False, x_T=None,
               verbose=True, timesteps=None, quantize_denoised=False,
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        log = dict()
        # z: resized noised low resolution images to latent space
        # c_lq: empty sentences
        # z_gt: real image to latent space
        # lr_con: real low resolution images without resize to latent space
        # x: low resolution image representations
        # gt: real image representations
        # yrec: decode z_gt to real image representations
        z, c_lq, z_gt, lr_con, x, gt, yrec, xc = self.get_input(batch, self.first_stage_key,
                                           return_first_stage_outputs=True,
                                           force_c_encode=True,
                                           return_original_cond=True,
                                           bs=N, val=True)
        ...
        if sample:
            ...
            with self.ema_scope("Plotting"):
                ...
                samples, z_denoise_row = self.sample(cond=c, struct_cond=struct_cond, lr_cond=lr_con, batch_size=N, timesteps=cur_time_step, return_intermediates=True, time_replace=self.time_replace)
            x_samples = self.decode_first_stage(samples)
            log["samples"] = x_samples
            if plot_denoise_rows:
                denoise_grid = self._get_denoise_row_from_list(z_denoise_row)
    ...
    #     params2 = list(self.structcond_stage_model.parameters())
    #     if self.learn_logvar:
    #         assert not self.learn_logvar
    #         print('Diffusion model optimizing logvar')
    #         params.append(self.logvar)
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, struct_cond=None, seg_cond=None, lr_cond=None, org_lr=None):
        if self.conditioning_key is None:
            ...
            if seg_cond is None:
                out = self.diffusion_model(x, t, context=cc, struct_cond=struct_cond, lr_cond=lr_cond, org_lr=org_lr)
            else:
