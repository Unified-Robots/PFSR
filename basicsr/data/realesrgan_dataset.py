...
class RealESRGANDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if 'ft_path' in opt:
            if isinstance(opt['ft_path'], str):
                self.ft_paths.extend(sorted([str(x) for x in Path(opt['ft_path']).glob('*.'+opt['image_type'])]))
            else:
                self.ft_paths.extend(sorted([str(x) for x in Path(opt['ft_path'][0]).glob('*.'+opt['image_type'])]))
                if len(opt['ft_path']) > 1:
                    for i in range(len(opt['ft_path'])-1):
                        self.ft_paths.extend(sorted([str(x) for x in Path(opt['ft_path'][i+1]).glob('*.'+opt['image_type'])]))
        ...
        if 'ft_path' in opt:
            self.paths =  60 * self.paths
            self.paths = self.paths + self.ft_paths
