...
class ImageSpliterTh:
    def __init__(self, im, lr_img, pch_size, stride, sf=1):
        '''
        Input:
        ...
        self.lr_stride = stride // sf
        self.lr_pch_size = pch_size //sf

        bs, chn, height, width= im.shape
        _, _, height_lr, width_lr = lr_img.shape

        self.height_starts_list = self.extract_starts(height)
        self.width_starts_list = self.extract_starts(width)

        # self.height_starts_list_lr = self.extract_starts_lr(height_lr)
        # self.width_starts_list_lr = self.extract_starts_lr(width_lr)
        self.height_starts_list_lr = [term // self.sf for term in self.height_starts_list]
        self.width_starts_list_lr = [term // self.sf for term in self.width_starts_list]
        self.length = self.__len__()
        self.num_pchs = 0
    ...
    def extract_starts_lr(self, length):
        if length <= self.lr_pch_size:
            starts = [0,]
        else:
            starts = list(range(0, length, self.lr_stride))
            for i in range(len(starts)):
                if starts[i] + self.lr_stride > length:
                    starts[i] = length - self.lr_pch_size
            starts = sorted(set(starts), key=starts.index)
        return starts
    ...
    def __next__(self):
        if self.num_pchs < self.length:
            ...
            w_start_lr = self.width_starts_list_lr[w_start_idx]
            w_end_lr = w_start_lr + self.lr_pch_size

            h_start_idx = self.num_pchs % len(self.height_starts_list)
            h_start = self.height_starts_list[h_start_idx]
            h_end = h_start + self.pch_size

            h_start_lr = self.height_starts_list_lr[h_start_idx]
            h_end_lr = h_start_lr + self.lr_pch_size

            pch = self.im_ori[:, :, h_start:h_end, w_start:w_end]
            lr_pch = self.im_lr[:, :, h_start_lr:h_end_lr, w_start_lr:w_end_lr]
        ...
        return pch, lr_pch, (h_start, h_end, w_start, w_end)
